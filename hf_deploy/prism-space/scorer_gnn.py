...
- Nodes: protein subunits, features = [ESM emb (640d) | pLDDT (5d) | SASA (1d) | geom (9d) | chirality (6d)] = 661d
- Edges: interface contacts, features = [ppi | esm_cos | go_cos | sasa_norm | contact_density | handedness] = 6d
- Output: per-candidate-addition compatibility score (raw logit for ranking CE loss)

v4 checkpoint (results/gnn_training/best_v4.pt): node_dim=655 (no chirality), edge_dim=5 (no handedness)
v5 checkpoint: node_dim=661 (+6d chirality node), edge_dim=6 (+1d chirality handedness)
...
class GATv2Layer(nn.Module):
    ...
    def forward(
        ...
        alpha_pre = F.softmax(e, dim=-1)
        alpha = self.drop(alpha_pre)
        ...
        if return_attn:
            return F.elu(out), alpha_pre.detach()
        return F.elu(out)
...
class MultiHeadGATv2(nn.Module):
    ...
    def forward(self, x, adj, edge_feats, return_attn: bool = False):
        if return_attn:
            outs, attns = [], []
            for h in self.heads_list:
                o, a = h(x, adj, edge_feats, return_attn=True)
                outs.append(o)
                attns.append(a)
            # attns: list of (N, N) → stack to (heads, N, N)
            return self.proj(torch.cat(outs, dim=-1)), torch.stack(attns, dim=0)
...
class AssemblyScorer(nn.Module):
    ...
    def embed(
        ...
        If return_attn=True, also returns list of (heads, N, N) attention tensors per layer."""
        ...
        for layer, norm in zip(self.layers, self.layer_norms):
            if return_attn:
                out, attn = layer(h, sparse_adj, edge_feats, return_attn=True)
                h = norm(h + out)
                all_attns.append(attn)  # (heads, N, N)
            else:
    ...
    def forward_with_attn(
        ...
        """Forward pass returning (scores, agg_attn).
        agg_attn: (N, N) float — mean across all layers and heads.
        Used by the viewer to map attention weights onto the 3D ribbon.
        """
        h, all_attns = self.embed(node_feats, adj, edge_feats, copy_indices, return_attn=True)
        scores = self.score_candidates(h, edge_feats, assembled_mask, candidates)
        # Aggregate: mean over heads per layer → mean over layers → (N, N)
        agg = torch.stack([a.mean(0) for a in all_attns]).mean(0)
        return scores, agg.cpu().tolist()
...
def build_node_features(
    ...
    train_gnn.py adds geom (9d) separately via np.hstack before calling build_node_features.
    Total dim here: 640 + 4(go) + 5(plddt) + 1(sasa) + 6(chirality) = 656d (subsets if absent).
    """
...
def build_edge_features(
    ...
    contact_density: Optional[np.ndarray] = None,  # (N, N) VoroIF Cα contact density
    handedness: Optional[np.ndarray] = None,        # (N, N) chirality det(R) ∈ {-1,0,+1} — CHIRALITY-SENSITIVE FEATURE
) -> torch.Tensor:
    """Stack pairwise scoring matrices into edge feature tensor (N, N, 4–6)."""
